                ...
                <li class="w-full flex items-center justify-between mt-5" v-for="({id, name, avatar}, index) in users" :key="id">
                    <div class="w-1/2 flex items-center">
                        <figure class="w-10 h-10 mr-3">
                            <img :src="avatar" alt="No existen favoritos" class="w-full h-full object-cover rounded-full"
                                loading="lazy">
                        </figure>
                        <div>
                    ...
                    <button v-if="!btnSend[index]" @click="sendMessage(index)"
                        class="'text-white py-1 px-7 rounded-lg font-bold bg-skyblue outline-0'">Enviar</button>
                    <button v-else
                        class="border-solid border-2 py-1 px-7 rounded-xl border-slate-400 font-bold outline-0">{{messageSend[index]}}</button>
                </li>
        </ul>
        <button class="bg-skyblue w-full fixed bottom-0 py-2 text-center font-semibold">Listo</button>
    </div>
</template>
